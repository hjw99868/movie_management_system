...
def show(request):
    accounts = backend.get_official_accounts()
    return render(request, 'show/index.html', {'accounts':accounts})
...
def render_ajax(request, url, params, item_id=''):
    if request.is_ajax():
        ...
        if identity == 'admin':
            ...
            applicationList = applications.filter(status__exact='approved')
            official_accounts = []
            for application in applicationList:
                official_accounts.append(backend.find_reverse(application))
...
def admin_dashboard(request):
    ...
    official_accounts = backend.get_official_accounts().order_by('-pk')[0:10]
...
def admin_show_articles_list(request):
    return render_sortable(request, backend.OfficialAccount.objects.all(),
...
def admin_show_applications(request, type):
    if type == 'pending':
        type_name = u'Applicatiosn on Dealing'
        type_icon = 'fa-tasks'
    elif type == 'processed':
        type_name = u'Applications Dealing by Me'
        type_icon = 'fa-check'
    elif type == 'all':
        type_name = u'All Applications'
...
def admin_show_official_account_articles_list(request, id):
    items = backend.get_articles_by_official_account_id(id)
    return render(request,'admin/detail/detail_articles_content.html', {'items': items})
