...
def find_reverse(app):
    return OfficialAccount.objects.get(application = app)
...
def modify_application(app):
    ...
    application.full_clean()
    application.save()
    account = application.official_account
    if application.status == 'approved' and settings.UPDATE_UPON_APPROVING:
        account.score = app['score']
        account.year = app['year']
        account.category = app['category']
        account.num = app['num']
        account.pic = app['pic']
        account.remark = app['remark']
    account.full_clean()
    account.save()
